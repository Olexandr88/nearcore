...
#### Watch out
When the un-limiting settings are applied to the nerwork config the network is prone to failing in non-obvoius ways.
It is better to keep an eye on missing chunks, and block production times for example.
...
This measures the performance of the runtime susystem in isolation applying the block or blocks in multiple configurable ways.
